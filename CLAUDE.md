...
frontend/
  ...
  js/
    ...
    audio.js                      Synthesized Web-Audio SFX (eat/dash/death); global `Sound`
                                   (NOT `Audio` — would shadow window.Audio). No asset files.
...
- `frontend/js/audio.js` produces **all** game sound by **synthesizing** it from Web Audio API oscillators (chiptune-style blips) — deliberately no audio asset files, matching the pixel look and the no-build-step frontend. It exposes a single global `Sound` object (named `Sound`, **not** `Audio`, which would shadow the built-in `window.Audio` constructor). Browsers block audio until a user gesture, so the `AudioContext` is created/resumed via `Sound.unlock()` from click handlers only (the Play/Join click in `doJoin()` and the mute button). Sound is **on by default** with a HUD mute toggle persisted in `localStorage["snakeAudioMuted"]`. Event hooks live in `main.js` at points that already exist — own score-up → `eat`, `dashing` false→true → `dashStart` (filtered-noise whoosh), the dash-ready edge in `updateDashMeter` → `dashReady`, `onGameOver` → `death` (timed to `DEATH_ANIM_MS`). The Play/Join click calls `Sound.unlock()`. All tunables are `AUDIO_*` constants in `frontend/js/config.js`. Only the local player's events make sound (others' eating/dying would be too noisy). There is deliberately no background music/ambient loop (tried, removed on user preference).
