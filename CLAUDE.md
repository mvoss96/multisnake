...
- Known gap, deliberately not addressed yet: there's no rate limiting or message-size limiting on the `/ws` connection itself (beyond Pydantic field validation). Acceptable for the current hands-on-demo scope; revisit before any longer-lived public deployment.
