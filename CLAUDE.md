...
- There is deliberately **no game audio**. A sound feature (synthesized Web-Audio SFX, then Kenney CC0 samples, plus a mute button and an ambient loop) was built and iterated on, but removed on user preference — don't reintroduce sound unless asked. The only leftover is defensive `width`/`height` on the dash-meter `.dash-progress` SVGs in `index.html` (prevents a page-filling black circle if CSS hasn't applied yet), which is unrelated and kept.
