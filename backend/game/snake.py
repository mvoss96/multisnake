...
class Snake:
    def __init__(self, snake_id, player_id, name, color, start_pos, start_direction, config):
        self.id = snake_id
    ...
    def set_desired_direction(self, angle):
    ...
    def move(self, dt, max_turn_rate):
        ...
        if self.dash_time_remaining > 0:
            ...
            if self.dash_charge < 1.0:
                self.dash_charge = min(1.0, self.dash_charge + dt / self.config.DASH_RECHARGE_SECONDS)

        heading = Vector2.from_angle(self.direction)
        head = self.head()
        new_head = Vector2(head.x + heading.x * current_speed * dt, head.y + heading.y * current_speed * dt)
    ...
    def grow(self, amount, score_value=1):
